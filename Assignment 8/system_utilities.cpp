...
int findDevice(string d){
    ...
    for (int i=0; i < numDevices; i++){
        if (devices[i]->amIThisDevice(d) == 1) {
            return curr_index;
        }
    curr_index++;
        }
    curr_index = -1; return curr_index;
    }

void processLeftTurnSignal(EVENT* ev){
    if (ev->getValue() == ON){
        devices[findDevice("Left Turn Lamp")]->setValue(ON);
        int new_time = globalTime + 2;
        EVENT* turn_off = new EVENT("Left Turn Lamp", OFF, new_time);
        eventList->insertEvent(turn_off);
        }
...
if (ev->getValue() == ON){
    ...
    int new_time = globalTime + 2;
    EVENT* turn_off = new EVENT("Right Turn Lamp", OFF, new_time);
    eventList->insertEvent(turn_off);
}
else if (ev->getValue() == OFF){
...
void processBrake(EVENT* ev){
    ...
    }else{devices[findDevice("Brake Lamps")]->setValue(OFF);}
}
...
void processLeftTurnLamp(EVENT* ev){
    ...
    int new_time = globalTime +2;
    EVENT* lft_event = new EVENT("Left Turn Lamp", new_val,new_time);
    eventList->insertEvent(lft_event);
}
void processRightTurnLamp(EVENT* ev){
    ...
    int new_time = globalTime +2;
    EVENT* lft_event = new EVENT("Right Turn Lamp", new_val,new_time);
    eventList->insertEvent(lft_event);
}
