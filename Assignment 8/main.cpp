/*
	A main function for testing assignment 8.
    ...
    do {
        ...
        switch(cmd) {
            ...
            case SET_DEVICE_VALUE:
            {
                ...
                if (devices[findDevice(parsed_command[1])]->myType() == DIGITAL_SENSOR || devices[findDevice(parsed_command[1])]->myType() == DIGITAL_CONTROLLER){
                    if(convertStringToValue(parsed_command[2]) == 1){
                        devices[findDevice(parsed_command[1])]->setValue(ON);
                    }else{devices[findDevice(parsed_command[1])]->setValue(OFF);}
                }else{
                    devices[findDevice(parsed_command[1])]->setValue(convertStringToValue(parsed_command[2]));
                }
